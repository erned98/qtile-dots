...
term         = 'alacritty'                                                          # terminal launch
term_class   = 'alacritty --class'                                                  # terminal launch wm_class
web          = 'qutebrowser'                                                        # web browser
rofi         = 'rofi -terminal alacritty -show'                                     # app launcher
...
keys = [
    ...
    Key([mod], "e", lazy.spawn(term + ' -e ranger')),                               # file manager
    Key([mod], "h", lazy.spawn(term + ' -e htop')),                                 # system monitor
    Key([mod], "m", lazy.spawn(term_class + ' audio -e ncmpcpp')),                  # local music player
    Key([mod, "shift"], "m", lazy.spawn(term_class + ' audio -e spotify_player')),  # spotify
    Key([mod], "p", lazy.spawn(term + '-e pulsemixer')),                            # volume control
    Key([mod, "shift"], "w", lazy.spawn(term_class + ' web -e newsboat -r')),       # RSS reader

    ## GUI apps
    Key([mod, "shift"], "b", lazy.spawn(web)),                                      # web browser
    Key([mod, "shift"], "g", lazy.spawn('gimp')),                                   # image editor
    Key([mod, "shift"], "d", lazy.spawn('brave --profile-directory=Default --app=https://discord.com/channels/@me')),       # damn discord

    ## other utilities
    Key([mod], "space", lazy.spawn(rofi + ' drun')),                                # app launcher
    Key([mod, "shift"], "space", lazy.spawn(rofi + ' run')),                        # run command
    Key([mod], "l", lazy.spawn('betterlockscreen -l')),                             # lock screen
...
groups = [
        ...
        Group("2", label="2", layout="max", matches=[Match(wm_class="qutebrowser"), Match(wm_class="web"), Match(wm_class="librewolf"), Match(wm_class="firefox"), Match(wm_class="Brave-browser")]),
        Group("3", label="3", layout="monadtall", matches=[Match(wm_class="audio")]), 
        ]
...
var_gap_left = 5
var_gap_right = 5
var_font_name = 'IosevkaTerm Nerd Font SemiBold'
    ...
    Screen(
        top=bar.Bar(
            [
                ...
                # workspace indicator
                widget.GroupBox(font='IosevkaTerm Nerd Font',
                                padding_x=5, 
                                center_aligned=True,
...
wl_input_rules = {
        ...
        "name:DELL08CA:00 04F3:30C4 Touchpad": InputConfig(tap=True, natural_scroll=True),
        }
